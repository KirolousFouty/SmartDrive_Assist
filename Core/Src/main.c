...
uint16_t row_pins[4] = {GPIO_PIN_8, GPIO_PIN_11, GPIO_PIN_4, GPIO_PIN_5};
...
uint16_t Read_ADC_X(void)
{
	...
	sConfig.Channel = ADC_CHANNEL_15; // IN15
...
// Read Y axis (ADC1 Channel IN16)
uint16_t Read_ADC_Y(void)
{
	ADC_ChannelConfTypeDef sConfig = {0};
	sConfig.Channel = ADC_CHANNEL_16; // IN16
	sConfig.Rank = ADC_REGULAR_RANK_1;
	sConfig.SamplingTime = ADC_SAMPLETIME_12CYCLES_5;
...
int main(void)
{
  ...
  while (1)
  {
		...
		uint16_t x_val = Read_ADC_X();
		uint16_t y_val = Read_ADC_Y();
